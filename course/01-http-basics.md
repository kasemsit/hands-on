...
## 1.1 HTTP คือการส่งจดหมายไป-กลับ
...
## 1.5 Status code ที่เจอบ่อยที่สุด
...
| `304` | Not Modified | ของเดิมยังใช้ได้ | ใช้ของใน cache (บทที่ 26) |
...
| `429` | Too Many Requests | ยิงถี่เกินไป | รอตาม `Retry-After` (บทที่ 12) |
...
## 1.8 อ่าน output จริงทีละบรรทัด
...
| ⑦ | `>` เปล่า ๆ | **บรรทัดว่างที่บอกว่า header จบแล้ว** (ตรงกับรูปในข้อ 1.3) |
...
1. **curl ใส่ header ให้เอง 3 ตัว** (④⑤⑥) โดยที่คุณไม่ได้สั่ง — ทั้งที่คำสั่งของคุณ
   ไม่มี `-H` เลยสักตัว
2. **บรรทัดว่าง ⑦ และ ⑪ คือสิ่งเดียวกับที่เห็นในรูปข้อ 1.3** ตอนนี้คุณเห็นมันของจริงแล้ว
...
## 1.9 ระดับ byte — HTTP/1.1 เป็น "ข้อความ" จริง ๆ
...
นี่คือ "บรรทัดว่าง" ที่พูดถึงตั้งแต่ข้อ 1.2 — ในระดับ byte มันคือ CR LF สองชุดติดกัน
...
นี่คือที่มาของบั๊กคลาสสิกที่จะเจอในบทที่ 21:
...
ต้องเข้ารหัสก่อน (ดูบทที่ 6.7 เรื่องชื่อไฟล์ภาษาไทย)
...
คนที่ดักกลางทางจะเห็นแต่ byte ที่เข้ารหัสแล้ว (บทที่ 7)
...
5. **จับ byte จริง**: ทำตามข้อ 1.9 ด้วย `nc -l -v` แล้วนับว่า request ของคุณยาวกี่ byte

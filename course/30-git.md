...
## 30.2 เริ่มใช้กับโปรเจกต์นี้เลย
...
> (บทที่ 20.4) โปรเจกต์นี้มี `.gitignore` เตรียมไว้ให้แล้ว
...
## 30.3 หกคำสั่งที่ใช้ 95% ของเวลา
...
## 30.5 กู้ของที่ทำพัง — 4 สถานการณ์
...
## 30.6 Branch — ลองของโดยไม่กลัวพัง
...
## 30.7 ⚠️ ความลับที่หลุดขึ้น git

นี่คือจุดที่คนทำ API พลาดกันบ่อยที่สุด และเชื่อมกับบทที่ 25.8 โดยตรง
...
> คอร์สนี้เก็บบทเรียนเป็น `.md` โดยตั้งใจ (บทที่ 30.10 ของแผน) เพราะ
> **GitHub render markdown และ mermaid ให้อ่านได้ทันที** โดยไม่ต้อง build อะไร

## 30.10 หาว่าอะไรทำให้พัง — `git bisect`
...
ถ้ามีเทสต์อัตโนมัติ (บทที่ 32) ใช้ `git bisect run pytest` ให้มันหาเองได้เลย
...
5. ตั้ง alias 4 ตัวในข้อ 30.8 แล้วลองใช้ `git s` กับ `git l`
6. สร้างไฟล์ที่มีคำว่า `API_KEY=sk_live_12345` แล้วลอง `git add` — คำสั่ง grep
   ในข้อ 30.7 จับได้ไหม

...
## 31.10 UDP คืออะไร — TCP ที่ตัดของทิ้ง {#s31-10a}

ตลอดบทนี้พูดถึง TCP (มี handshake, รับประกันว่าข้อมูลถึงครบและเรียงถูก)
**UDP คือ TCP ที่ตัดการรับประกันเหล่านั้นทิ้งเพื่อแลกความเร็ว**

| | **TCP** | **UDP** |
|---|---|---|
| handshake | ✅ 3 ขั้นก่อนส่ง ([ข้อ 31.3](#s31-3)) | ❌ ยิงได้เลย |
| ข้อมูลถึงครบไหม | ✅ รับประกัน · ส่งซ้ำถ้าหาย | ❌ **หายก็หายเลย** |
| เรียงลำดับถูกไหม | ✅ รับประกัน | ❌ อาจสลับ |
| ความเร็ว | ช้ากว่า (มี overhead) | **เร็วกว่า** |
| เปรียบเทียบ | จดหมายลงทะเบียน — มีใบตอบรับ | โปสการ์ด — ส่งแล้วส่งเลย |

**UDP เหมาะกับงานที่ "ช้าแล้วไร้ความหมาย" มากกว่า "หายแล้วเสียหาย":**

| งาน | ใช้ | ทำไม |
|---|---|---|
| โหลดเว็บ, ดาวน์โหลดไฟล์ | TCP | หายไม่ได้ ต้องครบ |
| **DNS query** ([ข้อ 31.8](#s31-8)) | UDP | query สั้น ถามใหม่ได้ถ้าไม่ตอบ |
| วิดีโอคอล, เกมออนไลน์ | UDP | เฟรมที่ช้ามาถึงก็ไร้ประโยชน์แล้ว ทิ้งดีกว่า |
| สตรีมสด | UDP | เหมือนกัน — ภาพเก่าไม่มีค่า |

> ## ทำไม "หายก็หายเลย" ถึงเป็นข้อดีได้
>
> ในวิดีโอคอล ถ้าเฟรมหนึ่งหาย TCP จะ**หยุดรอส่งใหม่** — กว่าจะได้มาภาพก็
> ค้างไปแล้ว ส่วน UDP ข้ามเฟรมนั้นไปเล่นเฟรมใหม่เลย **ภาพกระตุกดีกว่าภาพค้าง**
>
> นี่คือหลักการเดียวกับ head-of-line blocking ที่ QUIC แก้ (ตารางข้างล่าง)

**ดูของจริง** — DNS ใช้ UDP:

```bash
sudo tcpdump -ni any udp port 53          # เห็น DNS query วิ่งบน UDP
```

## 31.11 UDP กับ QUIC — ทำไมเริ่มสำคัญ {#s31-10}

...
## 31.2 การเดินทางของ request หนึ่งครั้ง
...
นี่คือเหตุผลที่ **connection reuse สำคัญมาก** (บทที่ 26.8) — ยิงครั้งที่สอง
บนการเชื่อมต่อเดิม ข้ามขั้น B, C, D ไปได้เลย

## 31.3 TCP handshake — สามขั้นตอน
...
## 31.4 ดูของจริงด้วย `ss` — ไม่ต้องเป็น root
...
## 31.5 สถานะของ TCP ที่ต้องรู้จัก
...
`[S]` = SYN, `[S.]` = SYN-ACK, `[.]` = ACK — **นั่นคือ handshake สามขั้นในข้อ 31.3**
ส่วน `[P.]` = PSH-ACK (ส่งข้อมูลจริง), `[F.]` = FIN, `[R]` = RST

## 31.7 Wireshark — เมื่อ tcpdump ไม่พอ
...
## 31.8 DNS — จุดที่พังบ่อยกว่าที่คิด
...
| `tcpdump` | ดู packet |
| `nmap` | สแกนว่าเปิด port อะไรบ้าง (**เครื่องตัวเองเท่านั้น** — บทที่ 22) |
| `curl -w` | วัดเวลาแต่ละช่วง (บทที่ 2.4) |
...
## 31.10 UDP กับ QUIC — ทำไมเริ่มสำคัญ
...
4. ใช้ `tcpdump -i lo -n -A port 8080` แล้วยิง curl — **อ่าน HTTP request
   ได้จากในนั้นไหม** เทียบกับสิ่งที่ `nc` ดักได้ในบทที่ 1.9

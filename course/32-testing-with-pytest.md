...
> บทนี้ปิดช่องว่างนั้น และเราจะเขียนเทสต์ **BOLA จากบทที่ 24** ที่ใช้ได้จริง
...
**เหตุผลที่หนักที่สุดสำหรับงานความปลอดภัย:** ช่องโหว่อย่าง BOLA (บทที่ 24)
...
> **ตั้งแต่ยังไม่ได้ยิงออกไปด้วยซ้ำ** ซึ่งตรงกับกฎในบทที่ 1.9 พอดี
...
**รายงานว่าล้มเหลว** (เพราะมันผ่านทั้งที่ควรพัง) เตือนว่าเอกสารบทที่ 24
ไม่ตรงกับโค้ดแล้ว
...
| 5 | **จำนวน query** (บทที่ 27.1) | กัน N+1 กลับมา |
...
ต่อยอด: `git bisect run pytest` (บทที่ 30.10) จะหาให้ว่า commit ไหนทำพัง
...
5. เขียนเทสต์ว่า `/api/solution` ปฏิเสธ payload ที่ไม่ใช่ base64 (บทที่ 16)

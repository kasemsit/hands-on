...
## 32.1 ทำไมต้องมีเทสต์อัตโนมัติ {#s32-1}
...
## 32.4 Fixture — ของกลางที่เตรียมให้อัตโนมัติ {#s32-4}
...
## 32.5 เทสต์เดียว หลายเคส — `parametrize` {#s32-5}
...
> **ตั้งแต่ยังไม่ได้ยิงออกไปด้วยซ้ำ** ซึ่งตรงกับกฎใน[บทที่ 1.9](01-http-basics.md#s1-9) พอดี
> — เทสต์จับกฎของ HTTP ให้เราโดยไม่ตั้งใจ

## 32.6 เทสต์ BOLA — ของจริงที่ควรมีทุกโปรเจกต์ {#s32-6}
...
## 32.7 `xfail` — เทสต์ที่ "ต้องพัง" {#s32-7}
...
## 32.9 เทสต์อะไรบ้างสำหรับ API {#s32-9}
...
| 5 | **จำนวน query** ([บทที่ 27.1](27-database-and-performance.md#s27-1)) | กัน N+1 กลับมา |
...
## 32.10 เทสต์ที่พังแบบสุ่ม (flaky) คือศัตรู {#s32-10}
...
| สาเหตุ | แก้ |
|--------|-----|
| `sleep()` ตายตัว | รอจนเงื่อนไขเป็นจริง (ดู[ข้อ 32.4](#s32-4)) |
...
ต่อยอด: `git bisect run pytest` ([บทที่ 30.10](30-git.md#s30-10)) จะหาให้ว่า commit ไหนทำพัง

...
| ที่ไหน | ทำอะไร |
|--------|--------|
| บทที่ 13 | ตรวจว่า webhook มาจากตัวจริง |
| บทที่ 16 | `signature` ใน ALTCHA challenge — พิสูจน์ว่าโจทย์เป็นของ server |
| บทที่ 12 | pagination cursor ที่ client แก้ไม่ได้ |

**สองกฎที่ห้ามลืม** (จากบทที่ 13.3):
...
| **อย่าเก็บในโค้ด** | secret manager (บทที่ 28.10) |
| **หมุนได้โดยไม่ downtime** | รองรับสองกุญแจพร้อมกันช่วงเปลี่ยนผ่าน |
| **มี `kid`** | บอกว่าใช้กุญแจใบไหน (บทที่ 10.3) |
...
ค่อยเอากุญแจเก่าออก (หลักการเดียวกับ backup pin ในบทที่ 7.6 และ webhook
secret ในบทที่ 13.6)

...
## 41.2 IOC vs IOA — ความต่างที่สำคัญ
...
**ข้อสุดท้ายมาจากระบบที่คุณสร้างเองในบทที่ 11** — reuse detection ไม่ได้แค่
ป้องกัน มันเป็น**สัญญาณเตือน**ด้วย ถ้ามันดังบ่อยผิดปกติแปลว่ามีคนกำลังเล่นงานคุณ

## 41.3 Log คืออะไรที่ต้องมีก่อนอย่างอื่น
...
## 41.5 ตั้ง alert ให้มีประโยชน์

โยงกับบทที่ 28.6 — **alert ที่ดีต้องมีคนต้องลงมือทำ**
...
## 41.6 กระบวนการ Incident Response
...
- [ ] รู้ว่าระบบไหนสำคัญที่สุด (จาก threat model — บทที่ 34)
...
> **การหมุน token ทั้งระบบทำได้เร็วแค่ไหน** — ถ้าคุณทำตามบทที่ 11
...
## 41.7 ตรวจเบื้องต้นเมื่อสงสัยว่าเซิร์ฟเวอร์โดน
...
- [ ] มี `security.txt` ให้คนภายนอกแจ้งเหตุได้ (บทที่ 22.6)

## แบบฝึกหัด

1. ดู `/var/log/auth.log` — มี `Failed password` กี่ครั้งใน 24 ชั่วโมง
   มาจาก IP ไหนมากที่สุด
2. รันคำสั่งตรวจทั้ง 6 ข้อในข้อ 41.7 บนเครื่องคุณ — มีอะไรที่อธิบายไม่ได้ไหม
...
5. เขียนกฎตรวจจับ (เป็นภาษาคนก็ได้) สำหรับ IOA 3 ข้อแรกในข้อ 41.2
...
8. เพิ่ม structured log + `request_id` ให้ lab server (แบบฝึกหัดค้างจากบทที่ 28)

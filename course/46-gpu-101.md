...
| `utilization.gpu` | % ของเวลาที่มี kernel ทำงาน | ⚠️ **หลอกตา** — ดูข้อ 46.7 |
...
## 46.3 ชั้นซอฟต์แวร์ — เวลาพังต้องรู้ว่าพังชั้นไหน
...
| `CUDA out of memory` | **VRAM ไม่พอ — บทที่ 47** |
...
## 46.4 VRAM คือทรัพยากรที่หมดก่อนเสมอ
...
## 46.5 ค่าใช้จ่ายที่มองไม่เห็น — CUDA context
...
- **รันหลาย process บน GPU เดียว = จ่ายค่า context หลายรอบ** — นี่คือเหตุผล
  ...
  (จะเจอเต็ม ๆ ในบทที่ 48)

## 46.6 ตัวเลขที่เปลี่ยนวิธีคิด — bandwidth
...
> **บทที่ 51 จะลงรายละเอียดเรื่องนี้** — และมันคือเหตุผลเดียวกับบทที่ 28.4
> ที่บอกว่า metric ที่ผิดทำให้ตัดสินใจผิด

## 46.8 ชนิดข้อมูล — ตัวแปรที่มีผลมากที่สุดต่อ VRAM
...
## 46.9 Tensor Core — หน่วยคำนวณที่ต้องใช้ให้เป็น
...
1. รัน `nvidia-smi --query-gpu=...` แบบในข้อ 46.2 บนเครื่องคุณ แล้วจดค่า
   `memory.total` กับ `memory.free` ไว้ — จะใช้ในบทที่ 47
2. วัดว่า CUDA context บนเครื่องคุณกินเท่าไร (โค้ดในข้อ 46.5)
3. วัด bandwidth PCIe vs VRAM (ข้อ 46.6) — ต่างกันกี่เท่า
...
***
[⬅ Supply chain security](42-supply-chain-security.md) · [สารบัญ](../README.md) · [VRAM และ KV cache ➡](47-gpu-memory-and-kv-cache.md)

...
## 49.1 ก่อนอื่น — คุณต้องการ "แบ่ง" จริงหรือเปล่า {#s49-1}
...
> ต้องการ **vGPU** ซึ่งเป็นคนละเรื่อง (ดู[ข้อ 49.10](#s49-10))

## 49.3 โควตาถูกบังคับที่ "ชั้นไหน" {#s49-3}
...
## 49.4 ⚠️ MIG มีเฉพาะบางการ์ด — ตรวจก่อนซื้อ {#s49-4}
...
## 49.5 พื้นฐานที่ต้องมีก่อน — NVIDIA Container Toolkit {#s49-5}
...
> ([บทที่ 46.3](46-gpu-101.md#s46-3)) — อัปเดต driver บน host กระทบทุก container พร้อมกัน

## 49.6 โหมดที่ 1 — Native {#s49-6}
...
พร้อม time limit บังคับ (ดู[ข้อ 49.12](#s49-12))

## 49.7 โหมดที่ 2 — MIG (ฮาร์ดแวร์แบ่ง) {#s49-7}
...
## 49.8 โหมดที่ 3 — HAMi (ซอฟต์แวร์แบ่ง) {#s49-8}
...
`libcuda.so` ([ข้อ 49.3](#s49-3)) ส่วน `LD_PRELOAD` คือกลไกของ Linux ที่สั่งว่า
...
## 49.9 โหมดที่ 4 — Time-slicing {#s49-9}

GPU สลับกันทำงานให้แต่ละ process เหมือน CPU สลับ process ([บทที่ 35.3](35-linux-internals.md#s35-3))
...
หัว[ข้อ 49.8](#s49-8) กับ 49.9 วางเรียงกันเหมือนเป็นทางเลือกที่เท่ากัน **ซึ่งทำให้เข้าใจผิด**
...
## 49.10 นอกเหนือจากสี่โหมด — MPS และ vGPU {#s49-10}
...
## 49.11 ต้องจ่าย NVIDIA AI Enterprise เมื่อไร {#s49-11}
...
[ข้อ 49.3](#s49-3) ที่ vGPU เป็นกลไกเดียวที่อยู่คนละเครื่องกับผู้ใช้
...
## 49.12 ชั้นที่แยกต่างหาก — Scheduler {#s49-12}
...
## 49.14 คำถามก่อนตัดสินใจซื้อ {#s49-14}
...
## 49.15 ประกอบเป็นระบบจริง {#s49-15}
...
9. ตอบคำถามใน[ข้อ 49.14](#s49-14) ทั้งหมดสำหรับโครงการที่คุณกำลังตัดสินใจอยู่

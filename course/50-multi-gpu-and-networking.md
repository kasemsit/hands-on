...
## 50.3 NCCL — ตัวที่ทำให้การ์ดคุยกัน
...
>
> นี่คือเหตุผลที่ต้องวัด ไม่ใช่คำนวณเอา (ข้อ 50.8)

## 50.4 ทางเชื่อม — ตัวเลขที่ต้องเห็นก่อนซื้อ
...
## 50.5 ดู topology ของเครื่องจริง
...
## 50.6 ⚠️ RTX PRO 6000 Blackwell ไม่มี NVLink
...
## 50.7 ข้ามเครื่อง — InfiniBand และ RoCE
...
(เทียบกับเส้นทางของ packet ปกติในบทที่ 31)
...
**เทียบผลที่ได้กับตารางข้อ 50.4** ถ้าได้ราว 50 GB/s = PCIe 5.0 ทำงานเต็มที่
...
| โมเดลใหญ่เกิน แต่ quantize ได้ | **quantize** ก่อนคิดเรื่องหลายใบ (บทที่ 47.8) |
...
2. ตรวจว่าการ์ดกำลังวิ่งที่ PCIe gen เท่าไร กี่เลน แล้วเทียบกับสเปกเมนบอร์ด
   ...
   nvidia-smi --query-gpu=pcie.link.gen.current,pcie.link.width.current --format=csv
   ```
3. ถ้ามีการ์ดสองใบ วัดแบนด์วิดท์ข้ามการ์ดด้วยสคริปต์ในข้อ 50.8

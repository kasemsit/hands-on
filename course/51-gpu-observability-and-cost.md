...
> [บทที่ 28](28-observability-and-deployment.md) สอนวัด API ด้วย RED metrics **บทนี้ทำแบบเดียวกันกับ GPU**
>
> และเริ่มด้วยข่าวร้าย: **ตัวเลขที่ทุกคนดูเป็นอันดับแรก คือตัวที่หลอกที่สุด**

## 51.1 `utilization.gpu` ไม่ได้แปลว่าที่คุณคิด {#s51-1}
...
## 51.2 DCGM — เครื่องมือที่ควรใช้จริง {#s51-2}
...
> ซึ่งมักเกิดจากใช้ fp32 แทน fp16/bf16 ([บทที่ 48](48-serving-llm.md)) — เป็นการทิ้งความเร็วไป
...
— โครงเดียวกับที่[บทที่ 28](28-observability-and-deployment.md) ใช้กับ API

**สำหรับ Kubernetes** ใช้ [GPU Operator](https://github.com/NVIDIA/gpu-operator)
ซึ่งติดตั้ง DCGM exporter ให้ทุก node อัตโนมัติ

## 51.3 วัดอะไรบ้างสำหรับงาน inference {#s51-3}

[บทที่ 48](48-serving-llm.md) แนะนำ 4 ตัวชี้ ทวนอีกครั้งพร้อมวิธีเก็บ
...
**เก็บ percentile ไม่ใช่ค่าเฉลี่ย** ([บทที่ 28.4](28-observability-and-deployment.md#s28-4)) — p50, p95, p99
เพราะค่าเฉลี่ยซ่อนคนที่รอนานที่สุดไว้

## 51.4 คิดต้นทุนต่อ token {#s51-4}
...
## 51.5 ตัวเลขที่ต้องเฝ้าในห้องแล็บจริง {#s51-5}
...
> ไม่ใช่ด้วยการซื้อการ์ดเพิ่ม ([บทที่ 49.1](49-gpu-on-containers-and-k8s.md#s49-1))

## 51.6 คอขวดมักไม่ได้อยู่ที่ GPU {#s51-6}
...
| GPU util สูง + power ต่ำ | ⚠️ kernel เล็กเกินไป ([ข้อ 51.1](#s51-1)) |
...
## 51.7 ⚠️ วัดผิดก็สรุปผิด — วัดให้นานพอ {#s51-7}
...
>
> ใน[บทที่ 48](48-serving-llm.md) lab วัด batching ก็ใช้หลักเดียวกัน — มี warm-up ก่อนจับเวลาเสมอ
...
## 51.8 ก่อนสรุปว่า "การ์ดไม่พอ" {#s51-8}
...
- [ ] **batch size เหมาะสมไหม** — batch เล็กเกินทำให้ throughput ตกหลายสิบเท่า
      ([บทที่ 48](48-serving-llm.md) วัดได้ 64.5 เท่า)
- [ ] **quantize แล้วหรือยัง** — int8 ให้พื้นที่เพิ่มเท่าตัว ([บทที่ 47.8](47-gpu-memory-and-kv-cache.md#s47-8))
...
10. วัดความเร็วเน็ตสองครั้ง ครั้งแรกโหลด 20 MB ครั้งที่สองโหลด 200 MB
    — ต่างกันเท่าไร อธิบายว่าทำไม ([ข้อ 51.7](#s51-7))

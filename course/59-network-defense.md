...
## 59.2 Firewall บน Linux — nftables {#s59-2}
...
## 59.3 🔍 ไล่ว่า packet ถูกตัดตรงไหน {#s59-3}
...
## 59.4 IDS/IPS — ตรวจจับสิ่งที่ผ่าน firewall เข้ามาแล้ว {#s59-4}
...
## 59.5 การเฝ้าดูที่ได้ผลจริงโดยไม่ต้องมี IDS {#s59-5}
...
## 59.6 ⚠️ ระวังการล็อกตัวเองออก {#s59-6}
...
2. ตั้ง `ufw deny out 443` แล้วลอง `curl https://example.com`
   — เห็นอะไรใน `tcpdump` (ข้อ [59.3](#s59-3))
...
5. ทำสคริปต์เทียบพอร์ตที่เปิดวันนี้กับเมื่อวาน (ข้อ [59.5](#s59-5)) แล้วตั้งเป็น cron
6. วาดผังเครือข่ายของระบบคุณ แล้วตอบว่าฐานข้อมูลออกอินเทอร์เน็ตได้ไหม —
   ถ้าได้ ทำไมถึงต้องได้
7. ฝึกตั้งตัวกู้อัตโนมัติในข้อ [59.6](#s59-6) แล้วจงใจตั้งกฎผิด — ระบบคืนค่าเองไหม
